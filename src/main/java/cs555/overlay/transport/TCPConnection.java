...
/**
 * Class to hold information about a socket connection
 * between the current node another node on the network.
 * Has functionality to send and receive messages over its
 * socket. Contains an active TCPReceiverThread which
 * automatically receives and parses messages.
 *
 * @author hayne
 */
public class TCPConnection {
    ...
    /**
     * The TCPReceiverThread object has been created, but a
     ...
     * been started, this will stop the thread as well.
     */
    public void close() {
        try {
            sender.dout.close();
            ...
            System.err.println( "Problem closing socket/streams. " 
                + ioe.getMessage() );
        }
    }
}
