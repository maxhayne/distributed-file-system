...
public class ChunkServer implements Node {
    ...
    public static void main( String[] args ) throws Exception {
        ...
        try ( ServerSocket serverSocket = new ServerSocket( serverPort );
            ...
            ( new Thread(
                new TCPServerThread( chunkServer, serverSocket ) ) ).start();

            System.out.println( "TCPServerThread is started and available at ["
                                + chunkServer.getHost() + ":"
                                + chunkServer.getPort() + "]" );
            ...
            chunkServer.sendGeneralMessage(
                Protocol.CHUNK_SERVER_SENDS_REGISTRATION,
            ...
            System.err.println(
                "ChunkServer failed to start. " + ioe.getMessage() );
            System.exit( 1 );
        }
    }
    ...
    public void onEvent( Event event, TCPConnection connection ) {
        ...
        switch ( event.getType() ) {
            ...
            default:
                System.err.println(
                    "Event couldn't be processed. " + event.getType() );
                break;
        }
    }
     ...
     * ChunkServer. If that function fails, and returns false, this method sends
     * a denial of the request to the requesting server.
     *
     * @param event
     * @param connection
     */
    private void serveFileToChunkServer( Event event,
        ...
        if ( !success ) {
            System.out.println(
                "serveFileToChunkServer: Unable to serve file." );
            try {
                sendGeneralMessage( Protocol.CHUNK_SERVER_DENIES_REQUEST,
                    filename, connection );
            } catch ( IOException ioe ) {
                System.err.println( "serveFileToChunkServer: Unable to send "
                                    + "denial of request. "
                                    + ioe.getMessage() );
            }
        }
    ...
    /**
     * Attempts to read a file from the disk, check to see that it isn't
     * corrupt, sends the data on to the requester and returns true. If it can't
     * read the file, or the file is corrupt, it returns false. Additionally, if
     * the file is corrupt, it adds an event to the fileService queue.
     *
     * @param filename
     * @param connection
     * @return
    ...
    /**
     * Calls a helper function, which returns true if the slices could be
     * served, and false if they couldn't. If false is returned, a message is
     * sent to the requester of the slices denying their request.
     *
    ...
    private void serveSlices( Event event, TCPConnection connection ) {
        ...
        if ( !success ) {
            ...
            try {
                ...
                System.out.println(
                    "serveSlices: Unable to send denial of request. "
                    + ioe.getMessage() );
            }
        }
    }
     ...
     * failures. If it doesn't fail, this function serves the slices it can to
     * the requester, and returns true.
     *
    ...
    private boolean serveSlicesHelper( RequestsSlices request,
        ...
        if ( servableSlices.isEmpty() ) {
            ...
            try {
                ...
                System.out.println( "serveSlicesHelper: Unable to serve "
                                    + "slices to requester. "
                                    + ioe.getMessage() );
            }
        }
    ...
    /**
     * Adds an event to the fileService queue stating that it should try to
     * acquire a specific file from a list ChunkServers provided in the
     * message.
     *
     * @param event
     * @param connection
     */
    private void acquireFile( Event event, TCPConnection connection ) {
        if ( connection == controllerConnection ) {
            ControllerRequestsFileAcquire request =
                ( ControllerRequestsFileAcquire ) event;
            fileService.addToQueue( request );

            try {
                sendGeneralMessage(
                    Protocol.CHUNK_SERVER_ACKNOWLEDGES_FILE_ACQUIRE,
                    request.filename, connection );
            } catch ( IOException ioe ) {
                System.out.println( "acquireFile: Unable to acknowledge "
                                    + "Controller's request to acquire '"
                                    + request.filename + "'. "
                                    + ioe.getMessage() );
            }
        } else {
            System.out.println( "acquireFile: Received a request to require a"
                                + " file, but it didn't come from the "
     ...
     * responses it receives are called 'pokeReplies', and a count of each is
     * kept in the ChunkServerConnection with this ChunkServer, and if the
     * discrepancy between pokes and pokeReplies becomes too great, the
     * ChunkServer is automatically deregistered.
     *
     * @param connection
     */
     ...
     * connection that requested it. If the helper function can't do that, for
     * any reason, it returns false, and this function sends a denial of the
     * request to the connection that sent it.
     *
     * @param event
     * @param connection
     */
    private void serveShard( Event event, TCPConnection connection ) {
        ...
        if ( !success ) {
            System.out.println( "serveShard: Unable to serve '" + filename
                                + "' to requester." );
            try {
                sendGeneralMessage( Protocol.CHUNK_SERVER_DENIES_REQUEST,
                    filename, connection );
                ...
                System.err.println(
                    "serveShard: Unable to send denial of request. "
                    + ioe.getMessage() );
            }
        }
    }
     ...
     * read without error, and the shard isn't corrupt, it serves the shard's
     * data to the requester and returns true. If the file cannot be read or the
     * shard is corrupt, it denies the request, adds an event to the fileService
     ...
     */
    private boolean serveShardHelper( String filename,
        TCPConnection connection ) {
        ...
        try {
            ...
            System.err.println( "serveShardHelper: Couldn't serve '" + filename
                                + "' to requester. " + ioe.getMessage() );
        }
        return true;
    }
    ...
    private void serveChunk( Event event, TCPConnection connection ) {
        ...
        if ( !success ) {
            System.out.println( "serveChunk: Unable to serve '" + filename
                                + "' to requester." );
            try {
                sendGeneralMessage( Protocol.CHUNK_SERVER_DENIES_REQUEST,
                    filename, connection );
                ...
                System.err.println(
                    "serveChunk: Unable to send denial of request. "
                    + ioe.getMessage() );
            }
        }
    }
    ...
    /**
     * Deals with a request for a chunk (which would come from another
     * ChunkServer or the Client). Attempts to read the file with the filename
     * sent in the message. Many things must go right to succeed. First, the
     * filename must be that of a properly formatted chunk, next, the file must
     * exist, the read must succeed, and the chunk mustn't be corrupted. If any
     * of those things don't happen, the read fails, false is returned and a
     * failure response is sent to the requester, and, if the failure happened
     * because the file is corrupt, the fileService will deal with the
     * corruption event.
     *
     * @param filename of chunk to serve
     * @param connection that requested the chunk
     */
    private boolean serveChunkHelper( String filename,
        TCPConnection connection ) {
    ...
    private void storeAndRelay( Event event, TCPConnection connection ) {
        ...
        try {
            sendGeneralMessage(
                Protocol.CHUNK_SERVER_ACKNOWLEDGES_FILE_FOR_STORAGE,
                message.filename, connection );
        } catch ( IOException ioe ) {
            System.err.println(
                "storeAndRelay: Unable to send acknowledgement to sender "
                + "of '" + message.filename + "'. " + ioe.getMessage() );
        }
        ...
        if ( fileBytes == null ) {
            ...
            System.out.println(
                "storeAndRelay: '" + message.filename + "' was " + not
                + "stored." );
        }
    ...
    /**
     * Attempts to delete a file from this ChunkServer based on a request from
     * the Controller.
     *
     * @param event
     * @param connection
    ...
    private void deleteRequestHelper( Event event, TCPConnection connection ) {
        String filename = ( ( GeneralMessage ) event ).getMessage();
        System.out.println(
            "deleteRequestHelper: Attempting to delete '" + filename
            + "' from the ChunkServer." );
        try {
            fileService.deleteFile( filename );
        } catch ( IOException ioe ) {
            System.out.println( "deleteRequestHelper: Could not delete file. "
                                + ioe.getMessage() );
        }
        try {
    ...
    /**
     * Interpret the response from the Controller to the registration request.
     * If successful, the ChunkServer's 'identifier' member will be set to the
     * one given by the Controller, the directory into which the ChunkServer
     * will store files will be created, and the FileDistributionService and
     * ChunkServerHeartbeatService will be started.
     *
     * @param event
     */
     ...
     * start the FileDistributionService and ChunkServerHeartbeatService in a
    ...
    private boolean registrationSetup( int identifier ) {
        try {
            ...
            fileService = new FileDistributionService( this );
            ( new Thread( fileService ) ).start();
            ChunkServerHeartbeatService heartbeatService =
                new ChunkServerHeartbeatService( this );
            ...
            heartbeatTimer.scheduleAtFixedRate( heartbeatService,
                randomOffset * 1000L, Constants.HEARTRATE );
        } catch ( Exception e ) {
            System.err.println(
                "registrationInterpreter: There was a problem setting up "
                + "the ChunkServer for operation after it had "
                + "been registered. " + e.getMessage() );
            if ( fileService != null ) {
                fileService.setActiveStatus( false );
                fileService = null;
            }
    ...
    private void interact() {
        ...
        while ( true ) {
            ...
            switch ( splitCommand[0].toLowerCase() ) {
                default:
                    System.err.println(
                        "Unrecognized command. Use 'help' command." );
                    break;
            }
        }
    }
