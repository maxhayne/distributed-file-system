...
public class Controller implements Node {
    ...
    public static void main( String[] args ) throws Exception {
        try ( ServerSocket serverSocket = new ServerSocket(
            ...
            ( new Thread(
                new TCPServerThread( controller, serverSocket ) ) ).start();

            System.out.println(
                "Controller's ServerThread has started at [" + host + ":"
                + Integer.toString( ApplicationProperties.controllerPort )
                + "]" );
            controller.interact();
        } catch ( IOException ioe ) {
            System.err.println(
                "Controller failed to start. " + ioe.getMessage() );
            System.exit( 1 );
        }
    ...
    public void onEvent( Event event, TCPConnection connection ) {
        switch ( event.getType() ) {
            ...
            case Protocol.CHUNK_SERVER_ACKNOWLEDGES_FILE_ACQUIRE:
                ...
                System.out.println( "onEvent: ChunkServer acknowledges that "
                                    + "it should acquire '" + filename + "'." );
    ...
    private void clientRead( Event event, TCPConnection connection ) {
        ...
        ControllerSendsStorageList response =
            new ControllerSendsStorageList( baseFilename, replications,
                shards );
    ...
    private void markFileFixed( Event event ) {
        ...
        if ( FileDistributionService.checkChunkFilename(
            ...
            System.err.println(
                "markFileFixed: '" + report.filename + "' is not"
    ...
    private void missingFileHelper( Event event ) {
        ...
        int sequence =
            Integer.parseInt( message.filename.split( "_chunk" )[1] );
        if ( identifier != -1 ) {
    ...
    private void corruptionHelper( Event event, TCPConnection connection ) {
        ...
        if ( FileDistributionService.checkChunkFilename( report.filename ) ) {
            ...
            System.err.println(
                "corruptionHelper: '" + report.filename + "' is not"
                + " a valid name for either a chunk or a shard." );
            return;
        }
        ...
        if ( info == null ) {
            System.err.println(
                "corruptionHelper: No servers could be found which"
                + " could help recreate '" + report.filename
        ...
        String[] shards = split[1].split( "," );
        ControllerSendsStorageList response =
            new ControllerSendsStorageList( report.filename, replications,
                shards );
        try {
            connection.getSender().sendData( response.getBytes() );
        } catch ( IOException ioe ) {
            System.err.println(
                "corruptionHelper: Unable to provide ChunkServer with"
    ...
    private void heartbeatHelper( Event event ) {
        ChunkServerSendsHeartbeat heartbeat =
            ( ChunkServerSendsHeartbeat ) event;
        ...
        if ( connection == null ) {
            System.err.println(
                "heartbeatHelper: there is no registered ChunkServer"
                + "with an identifier of " + heartbeat.identifier + "." );
            return;
        }
        connection.getHeartbeatInfo()
            .update( heartbeat.type, heartbeat.freeSpace, heartbeat.totalChunks,
                heartbeat.files );
    }
    ...
    private void storeChunk( Event event, TCPConnection connection ) {
        ...
        String[] servers =
            connectionCache.availableChunkServers( request.filename,
                request.sequence ).split( "," );

        // Choose which response to send
        Event response;
        if ( servers[0].equals( "" ) ) { // failure
            response = new GeneralMessage(
                Protocol.CONTROLLER_DENIES_STORAGE_REQUEST );
        } else { // success
            response =
                new ControllerSendsClientValidChunkServers( request.filename,
    ...
    private void storeShards( Event event, TCPConnection connection ) {
        ...
        String[] servers =
            connectionCache.availableShardServers( request.filename,
                request.sequence ).split( "," );
        ...
        if ( servers[0].equals( "" ) ) { // failure
            ...
            response =
                new ControllerSendsClientValidShardServers( request.filename,
                    request.sequence, servers );
        }
    ...
    private void registrationHelper( Event event, TCPConnection connection,
        ...
        if ( type ) { // attempt to register
            String host = request.getMessage().split( ":" )[0];
            int port = Integer.parseInt( request.getMessage().split( ":" )[1] );
            int registrationStatus = connectionCache.register( host, port,
                connection ); // attempt to register
            ...
            connectionCache.deregister(
                Integer.parseInt( request.getMessage() ) );
        }
    }
